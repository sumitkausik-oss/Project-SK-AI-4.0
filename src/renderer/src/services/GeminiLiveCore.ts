...
export class GeminiLiveCore {
  ...
  async connect(apiKey: string, callbacks: LiveEngineCallbacks) {
    ...
    try {
      ...
      this.ws.onerror = (err) => {
        ...
        callbacks.onError('AI कनेक्शन में समस्या आई। API Key की जांच करें।')
      }
  ...
  sendTextMessage(text: string, callbacks: LiveEngineCallbacks) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      callbacks.onError('कृपया पहले "START 2-WAY VOICE" पर क्लिक करके AI कनेक्ट करें।')
      return
    }
  ...
  private sendSetupEnvelope() {
    const envelope = {
      setup: {
        ...
        systemInstruction: {
          parts: [{
            text: 'You are SK AI, an autonomous intelligence built for Sumeet Kumar (Powered by SK Enterprises). You converse fluently, respectfully, and crisply in Hindi / Hinglish. When the user asks to open D drive, apps, take screenshots, or inspect files, ALWAYS invoke the corresponding tool function immediately and confirm verbally in Hindi.'
          }]
        },
        tools: [{
          functionDeclarations: [
            {
              ...
              parameters: {
                type: 'OBJECT',
                properties: { target: { type: 'STRING', description: 'Drive letter (D:, C:) or folder path' } },
                required: ['target']
              }
            },
            {
